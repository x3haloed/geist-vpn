...
pub fn view<'a>(
    ...
    let header_row =
        row![title, iced::widget::horizontal_space(), add_button].align_y(iced::Alignment::Center);
    ...
    let content = if loading {
        ...
        let filtered_profiles: Vec<&VpnProfile> = match current_view {
            ...
            ViewMode::Recent => {
                ...
                let mut recent: Vec<&VpnProfile> = profiles
                    .iter()
                    .filter(|p| p.metadata.last_used_at.is_some())
                    .collect();
                recent.sort_by(|a, b| b.metadata.last_used_at.cmp(&a.metadata.last_used_at));
        ...
        if filtered_profiles.is_empty() {
            ...
            let profiles_column = Column::with_children(profile_elements).spacing(8);

            let scrollable_content = scrollable(profiles_column).height(Length::Fill);

            column![header_row, scrollable_content]
                .spacing(12)
                .padding(16)
        }
    };
...
fn create_profile_element<'a>(profile: &'a VpnProfile) -> Element<'a, Message> {
    ...
    let profile_info = column![
        text(format!(
            "{} {}",
            profile.name,
            if profile.metadata.favorite { "⭐" } else { "" }
        ))
        .size(16)
        .style(|theme: &iced::Theme| iced::widget::text::Style {
            color: Some(theme.palette().text),
        }),
        text(format!("{} • {:?}", profile.host, profile.protocol)).style(|theme: &iced::Theme| {
            iced::widget::text::Style {
                color: Some(theme.palette().text),
            }
        }),
        text(&profile.description).style(|theme: &iced::Theme| iced::widget::text::Style {
            color: Some(theme.palette().text),
        })
    ]
    .spacing(4);
    ...
    let metadata_info = if let Some(last_used) = &profile.metadata.last_used_at {
        column![
            text(format!("Last used: {}", last_used.format("%Y-%m-%d %H:%M"))).style(
            ...
            text(format!(
                "Used {} time{}",
                profile.metadata.usage_count,
                if profile.metadata.usage_count == 1 {
                    ""
                } else {
                    "s"
                }
        ...
        column![
            text("Never used").style(|theme: &iced::Theme| iced::widget::text::Style {
                color: Some(theme.palette().text),
            })
        ]
    };

    let info_column = column![profile_info, metadata_info].spacing(8);

    let action_buttons = row![favorite_button, edit_button, delete_button].spacing(8);
