...
pub fn view(connection_status: &ConnectionStatus) -> Element<Message> {
    ...
    let status_indicator = container(
        row![
            text("●").style(move |theme: &iced::Theme| text::Style {
            ...
            text(&connection_status.status_message).size(16)
        ]
        .spacing(8)
    ...
    let header_row = row![title, iced::widget::horizontal_space(), status_indicator]
        .align_y(iced::Alignment::Center);
